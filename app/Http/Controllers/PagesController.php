...
class PagesController extends Controller
{
    ...
    public function postTintucTimkiem(Request $request,$menu_slug)
    {
    ...
    public function postTuyendungTimkiem(Request $request,$menu_slug)
    {
        $menu = Menu::where('slug',$menu_slug)->first();
        $tukhoa = $request->tukhoa;
        if ($tukhoa != "") {
            ...
            $content_new = Content::where('menu_id',$menu->id)->where('status',1)->orderBy('created_at','desc')->take(5)->get();
            $content->appends(['tukhoa' => $tukhoa ]);
            return view('shared.pages.noidung.tuyendung.search',compact('menu','tukhoa','content','content_new'));
        } else {
            return redirect()->back();
        }
